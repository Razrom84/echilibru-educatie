...
- Fără streak / badge / scor. Fără chat. Personajele sunt SVG statice. S1 **`Căsuță`** (ă, ț, ă — nu `Cănuță`), S2 **`Găletușă`** (ă, ș, ă — nu `Găletuță`). Personajul S4 se afișează **`Mânuță`** (â din *mână*, U+00E2 — nu `Mănuță`; id intern `manuta`). S5 **`Cariocă`** (ă), S6 **`Săgeată`** (ă), S7 **`Preșuleț`** (ș, ț), S8 **`Frunzuliță`** (ț, ă), S9 **`Suflare`**, S10 **`Cutiuță`** (ț, ă), S11 **`Luminiță`** (ț, ă), S12 **`Cănuță`** (ă, ț, ă — nu `Mânuță`), S13 **`Hăinuță`** (ă, ț, ă), S14 **`Potecuță`** (ț, ă), S15 **`Linguriță`** (ț, ă), S16 **`Picătură`** (ă, ă), S17 **`Păsărică`** (ă, ă, ă), S18 **`Mingiuță`** (ț, ă), S19 **`Cărticică`** (ă, ă), S20 **`Coșuleț`** (ș, ț). S21 **`Ferestruică`** (ă), S22 **`Ghemotoc`**, S23 **`Năsuc`** (ă), S24 **`Ușiță`** (ș, ț, ă), S25 **`Lampadar`**, S26 **`Amintire`**, S27 **`Fulgușor`** (ș), S28 **`Murdărel`** (ă), S29 **`Muguraș`** (ș), S30 **`Cioculeț`** (ț), S31 **`Sămânțică`** (ă, â, ț, ă), S32 **`Balonaș`** (ș), S33 **`Lopățică`** (ă, ț, ă), S34 **`Umbriță`** (ț, ă), S35 **`Stropuleț`** (ț), S36 **`Gândăcel`** (â, ă), S37 **`Răcorică`** (ă, ă), S38 **`Tălpiță`** (ă, ț, ă), S39 **`Merișor`** (ș), S40 **`Măturică`** (ă, ă), S41 **`Portiță`** (ț, ă), S42 **`Vântuleț`** (â, ț), S43 **`Săculeț`** (ă, ț), S44 **`Inimioară`** (ă), S45 **`Plimbărel`** (ă), S46 **`Degețel`** (ț), S47 **`Grijuliul`**, S48 **`Norocel`**, S49 **`Scumpicel`**, S50 **`Cumințel`** (ț), S51 **`Gospodărel`** (ă), S52 **`Blânduț`** (â, ț).
